...
public class TipoidentificacionFacadeREST extends AbstractFacade<Tipoidentificacion> {

    @PersistenceContext(unitName = "co.edu.javeriana_UniversityApp_war_1.0-SNAPSHOTPU")
    private EntityManager em;

    public TipoidentificacionFacadeREST() {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Tipoidentificacion entity) {
        super.edit(entity);
    }
    ...
    public Tipoidentificacion find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Tipoidentificacion> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
