...
@Stateless
@Path("section")
public class CourseSectionFacadeREST extends AbstractFacade<Section> {

    @PersistenceContext(unitName = "co.edu.javeriana_UniversityApp_war_1.0-SNAPSHOTPU")
    private EntityManager em;

    public CourseSectionFacadeREST() {
        super(Section.class);
    }
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, Section entity) {
        super.edit(entity);
    }
    ...
    public Section find(@PathParam("id") Long id) {
    ...
    @Path("{id}/students")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Student> getStudents(@PathParam("id")Long sectionId) {
        return find(sectionId).getStudents();
    }
